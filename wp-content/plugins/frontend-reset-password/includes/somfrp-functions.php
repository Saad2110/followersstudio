...
function somfrp_lost_pass_handler( $action = '' ) {
	...
	if ( ! somfrp_verify_nonce_request( 'somfrp_lost_pass' ) ) {
		somfrp_wp_error( '<strong>' . esc_html__('ERREUR', 'frontend-reset-password') . ':</strong> ' . esc_html__( 'something went wrong with that!', 'frontend-reset-password' ) );
	}
	...
	if ( isset( $user_info ) && ! empty( $user_info ) ) {
		...
		if ( strpos( $user_info, '@' ) ) {
			...
			if ( empty( $user_data ) ) {
				$errors['no_email'] = esc_html__( "Cette adresse email n'est pas reconnue.", 'frontend-reset-password' );
			}
		} else {
...
function somfrp_lost_pass_callback() {
	...
	if ( empty( $user_info ) ) {
		...
		somfrp_wp_error( '<strong>' . esc_html__('ERREUR', 'frontend-reset-password') . ':</strong> ' . esc_html__( 'Please add your email address or username!', 'frontend-reset-password' ) );
		exit;
		...
		$user_data = get_user_by( 'email', $user_info );
		if ( empty( $user_data ) ) {
			somfrp_wp_error( '<strong>' . esc_html__('ERREUR', 'frontend-reset-password') . ':</strong> ' . esc_html__( 'Aucune adresse e-mail trouvée !', 'frontend-reset-password' ) );
			exit;
		}
		...
		$user_data = get_user_by( 'login', $user_info );
		if ( empty( $user_data ) ) {
			somfrp_wp_error( '<strong>' . esc_html__('ERREUR', 'frontend-reset-password') . ':</strong> ' . esc_html__( "Aucun nom d'utilisateur trouvé !", 'frontend-reset-password' ) );
			exit;
		}
	...
	if ( is_wp_error( $key ) ) {
		somfrp_wp_error( '<strong>' . esc_html__('ERREUR', 'frontend-reset-password') . ':</strong> ' . esc_html__( 'clé introuvable!', 'frontend-reset-password' ) );
		//return $key;
	}
